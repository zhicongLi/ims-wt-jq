	...
	function addFilterToJquery($ajaxCache) {
	    ...
	    $.ajaxPrefilter(function (options, originalOptions, jqXHR) {
	        ...
	        var ajaxCacheOptions = options.ajaxCache;
             ...
             if(options.url.indexOf('listDataStr?type')>=0) {
                 ajaxCacheOptions=true;
			 }
	    ...
	    */
	    $.ajaxTransport("+*", function (options, originalOptions, jqXHR) {
	        var cacheProxy = $ajaxCache.getCacheProxy();
	        var ajaxCacheOptions = options.ajaxCache;
            if(options.url.indexOf('dict/listDataStr')>=0
				||options.url.indexOf('sysObjCtrl/myObjCtrl')>=0
				||options.url.indexOf('menu/listByType')>=0
			) {
                ajaxCacheOptions=true;
            }
	        if (ajaxCacheOptions) {
