...
{
	var curTableRandom = Math.random()+100;//当前table对应随机数
	var newTableRandom = Math.random()+10;//新拆分的table对应随机数
	var wtCode = "";
//		var pageBreakFlag = false;
	...
	var pageHeight = A4;//内容高度，要减去页面所占高度  
...
//			{
			 ...
			 if(breakPage.length == 0)
			{
			 ...
			 if(breakPage.length == 0)
			{
				 blockTop = table.parent().offset().top - table.parent().prevAll(".pageHeader:first").height();
			}
			 else
			{
...
//			}
		...
		for(var i = 0;i < trs.length;i++)
		{
			...
			if((trTop - blockTop) > pageHeight)
			{
				...
				{
					table = $(table).prev();
					curTableRandom = Math.random()+100;//table变化对应随机数变化
					trs = table.find("tr");
					i = trs.length - 1;
					row = trs[i];
				}
				...
				if($(row).prev().length == 0)
				{
					...
					table.before("<div class='pageFooter'></div><div class='pageBreak'></div><div class='pageHeader'></div>");
				...
				{
					//设置每个td为原来的宽度，避免合并单元格行拆分时出现宽度异常,一个table最多设置一遍
					if(curTableRandom != newTableRandom)
					{
						var curTds = nextAllRow[i].children;
						for(var k = 0;k < curTds.length;k++)
						{
							$(curTds[k]).css("width",$(curTds[k]).width());
						}
					}
...
//					pageBreakFlag = true;
				...
				table.after("<div class='pageFooter'></div><div class='pageBreak'></div><div class='pageHeader'></div>");
				table.after(getPageInfo(pageNo,sign));
				pageNo++;
				//after加了6个所以next6次
				table = table.next().next().next().next().next().next();
				flag = true;
				breakFlag = true;
				getNextTable = false;
				
				newTableRandom =  Math.random()+10;
				curTableRandom = newTableRandom;
				break;
			}
		}
		//table = table.next();
		if(table.next().length == 0)
		{
			...
			//var top = pageNo * pageHeight - $("#lastMainPage").offset().top;

			var top = pageHeight - (lastPage.offset().top - blockTop) - lastPage.height();
			if(top<0)
				top=0;
		...
		{
			table = table.next();
			//table变化，当前table对应随机数跟随变化
			curTableRandom = Math.random()+100;
			flag = true;
		}
	}
	return pageNo;
...
function exitPreForm(){
	location.href="http://127.0.0.1:8181/treps/a/screen/wo/woWt/selfPrintForm?userName=admin&token=45a100f9a34f022831808c2831a24243d4e187e4932bcc39f567d09b";
