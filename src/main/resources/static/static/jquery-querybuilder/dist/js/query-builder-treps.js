...
var QueryBuilder = function($el, options) {
    ...
    //treps
    // for(var i=0;i<QueryBuilder.OPERATORS.length;i++){
    //     QueryBuilder.OPERATORS[i]['label']=this.lang.operators[QueryBuilder.OPERATORS[i].type];
    // }
...
QueryBuilder.OPERATORS = {
    smart:            { type: 'smart',            nb_inputs: 1, multiple: false, apply_to: ['string', 'number', 'datetime', 'boolean'] },
    equal:            { type: 'equal',            nb_inputs: 1, multiple: false, apply_to: ['string', 'number', 'datetime', 'boolean'] },
    not_equal:        { type: 'not_equal',        nb_inputs: 1, multiple: false, apply_to: ['string', 'number', 'datetime', 'boolean'] },
    'in':               { type: 'in',               nb_inputs: 1, multiple: true,  apply_to: ['string', 'number', 'datetime'] },//treps
...
QueryBuilder.DEFAULTS = {
    ...
    icons: {
        add_group:    'fa-plus',
        add_rule:     'fa-plus',
        remove_group: 'fa-remove',
        remove_rule:  'fa-remove',
        error:        'fa-warning-sign'
    }
};
...
QueryBuilder.prototype.bindEvents = function() {
    ...
    // rule operator change 转移至addOperator
...
QueryBuilder.prototype.setRoot = function(addRule, data, flags) {
    ...
    //treps
    mini.parse();
    // var groupTemp=mini.get( group_id+"_cond");
    // groupTemp.setData([ {id:'AND',label:'和'},
    //     {id:'OR',label:'或'}
    // ]);
    // groupTemp.on('valuechanged',  function() {
    //
    //     self.getModel($group).condition = this.getValue();
    // });
    // var temp_control  = mini.get(group_id+"_invert_group" );
    // temp_control.on('click',  function() {
    //     //var $group = $(this).closest(Selectors.group_container);
    //     self.invert(self.getModel($group), {});
    // });
    // temp_control  = mini.get(group_id+"_group_not" );
    // temp_control.on('click',  function() {
    //     var group = self.getModel($group);
    //     group.not = !group.not;
    // });
    self.groupBindActions( $group,group_id);
...
QueryBuilder.prototype.addGroup = function(parent, addRule, data, flags) {
    ...
    QueryBuilder.prototype.groupBindActions = function( groupEl,group_id) {
        var self=this;
        var groupTemp=mini.get(group_id+"_cond");
        groupTemp.setData([ {id:'AND',label:'和'},
            {id:'OR',label:'或'}
        ...
        groupTemp.on('valuechanged',  function() {
            ...
            self.getModel(groupEl).condition = this.getValue();
        });
        //console.info('conditions',this.conditions);
        ...
        temp_control.on('click',  function() {
            //var $group = $(this).closest(Selectors.group_container);
            self.invert(self.getModel(groupEl), {});
        });
        temp_control  = mini.get(group_id+"_group_not" );
        temp_control.on('click',  function() {
            var group = self.getModel(groupEl);
            group.not = !group.not;
        });
    };
/**
...
QueryBuilder.prototype.updateGroupCondition = function(group, previousCondition) {
    ...
    // });
    var group_cond=mini.get(group.id+"_cond");
    group_cond.setValue(group.condition);
...
QueryBuilder.prototype.addRule = function(parent, data, flags) {
    ...
    var temp_controls = mini.getsbyName("_invert_rule");

    $.each(temp_controls,function(i,item) {
        //var filterId = item.getId();
        item.on('click',  function() {
            //var $rule = $(this).closest(Selectors.rule_container);
            self.invert(self.getModel($rule), null);
        });
    });
    if (data !== undefined) {
        model.data = data;
...
QueryBuilder.prototype.createRuleFilters = function(rule) {
    ...
    //console.info($filterSelect);
    mini.parse();
    var  filterSelectTemp=mini.get(rule.id+'_filter');
    filterSelectTemp.setData(filters);
    // rule filter change
    var qbTemp=this;
    filterSelectTemp.on('valuechanged',  function() {
        //console.info('valuechanged',this.getValue());
        rule.filter=self.getFilterById(this.getValue());
        //filterSelectTemp.setValue(this.getValue());
        //self.updateRuleFilter(rule);
    });
    /**
...
QueryBuilder.prototype.createRuleOperators = function(rule) {
    ...
    mini.parse();
    var  operatorSelectTemp=mini.get(rule.id+'_operator');
    operatorSelectTemp.setData(operators);
    operatorSelectTemp.on('valuechanged',  function() {
        //var $rule = $(this).closest(Selectors.rule_container);
        rule.operator = self.getOperatorByType(this.getValue());
    });
    operatorSelectTemp.select(0);
    operatorSelectTemp.focus( );
    ...
    //rule.$el.find(QueryBuilder.selectors.rule_operator).val(rule.operator.type);
     operatorSelectTemp.setValue(rule.operator.type);
    /**
...
QueryBuilder.prototype.createRuleInput = function(rule) {
    ...
    for (var i = 0; i < rule.operator.nb_inputs; i++) {
        ...
        var inputTemp=mini.get(name);
        inputTemp.on('valuechanged'  , function() {
            if (!rule._updating_input) {
                rule._updating_value = true;
                rule.value = self.getRuleInputValue(rule);
                rule._updating_value = false;
                console.info('valuechanged',rule.value);
            }
        });
...
QueryBuilder.prototype.updateRuleFilter = function(rule, previousFilter) {
    ...
    console.info('updateRuleFilter',rule);
    ...
    var out = (function parse(group) {
        ...
        group.each(function(rule) {
            ...
            var ruleData = {
                ...
                value: value,
                tableName: rule.filter.tableName,//treps add
                fK: rule.filter.fK,
                idField: rule.filter.idField,
                valueField: rule.filter.valueField
            };
...
QueryBuilder.prototype.getOperators = function(filter) {
    ...
    for (var i = 0, l = this.operators.length; i < l; i++) {
        ...
        //treps
        this.operators[i]['label']=this.lang.operators[this.operators[i].type]
    ...
    if (filter.valueGetter) {
        ...
        for (var i = 0; i < operator.nb_inputs; i++) {
            ...
            switch (filter.input) {
                case 'radio':
                case 'checkbox':
                    value.push(inputTemp.getChecked());
                    break;



                default:
                    value.push(inputTemp.getValue());
            }
        }
...
<div id="{{= it.group_id }}" class="rules-group-container"> \
  <div class="rules-group-header"> \
    <div  name="{{= it.group_id }}_cond" id="{{= it.group_id }}_cond"  style="float:left"  class="mini-radiobuttonlist" repeatItems="1" repeatLayout="table" textField="label" valueField="id"></div>\
    <div class="btn-group pull-right group-actions"> \
      <a  class="mini-button" iconCls="" id="_add_rule"> \
       <i class="fa fa-plus"></i> {{= it.translate("add_rule") }} \
      </a> \
      {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }} \
        <a  class="mini-button" iconCls="" id="_add_group"> \
          <i class="fa fa-plus"></i> {{= it.translate("add_group") }} \
        </a> \
      {{?}} \
      {{? it.level>1 }} \
        <a  class="mini-button" iconCls="" id="_delete_group"> \
          <i class="far fa-trash-alt"></i> {{= it.translate("delete_group") }} \
        </a> \
      {{?}} \
    </div> \
    <div class="btn-group group-conditions" > \
          <div  name="{{= it.group_id }}_cond" id="{{= it.group_id }}_cond"  style="float:left" class="mini-radiobuttonlist" repeatItems="1" repeatLayout="table" repeatDirection="vertical" textField="label" valueField="id"></div>\
    </div> \
...
<div id="{{= it.rule_id }}" class="rule-container"> \
  <div class="rule-header"> \
    <div class="btn-group pull-right rule-actions"> \
      <a  class="mini-button" id="_delete_rule" data-delete="rule" iconCls="" >  \
        <i class="far fa-trash-alt"></i> {{= it.translate("delete_rule") }} \
      </a> \
    </div> \
  </div> \
  {{? it.settings.display_errors }} \
...
{{ var optgroup = null; }} \
<input class="mini-combobox" name="{{= it.rule.id }}_filter" id="{{= it.rule.id }}_filter" valueField="id" textField="label"> \
</input>';
...
{{?}} \
{{ var optgroup = null; }} \
<input class="mini-combobox" name="{{= it.rule.id }}_operator" id="{{= it.rule.id }}_operator" valueField="type" textField="label"> \
</input>';

QueryBuilder.templates.ruleValueSelect = '\
{{ var optgroup = null; }} \
<input class="mini-combobox" name="{{= it.name }}" {{? it.rule.filter.multiple }}multiple{{?}}> \
</input>';
...
QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
    var filter = rule.filter;
    var colFilter = rule.filter.col.filter;
    //var colFilter = rule.filter.col.filter;
    var validation = rule.filter.validation || {};
    ...
    }else if(rule.filter.col!=undefined && rule.operator.type=='smart'){
        switch (colFilter.type) {
            case 'combobox':
                h += '<input id="'+name+'" name="'+name+'"   class="mini-combobox" multiSelect="'+colFilter.multiSelect+'" valueField="'+colFilter.valueField+'" textField="'+colFilter.textField+'"  url="'+colFilter.url+'" vtype = "'+colFilter.vtype+'"   style="'+colFilter.style+'" ';
                h +='  showClose="'+colFilter.showClose+'" oncloseclick="onFilterCloseInter(this)" ';
                //h +='   onvaluechanged="onFilterChangedInter" ';
                h +=' />';
                break;
            case 'textbox':
                h += '<input id="'+name+'" name="'+name+'"   class="mini-textbox"  style="'+colFilter.style+'" ';
                h +='  showClose="'+colFilter.showClose+'" oncloseclick="onFilterCloseInter(this)" ';
                //h +='   onvaluechanged="onFilterChangedInter" ';
                h +=' />';
                break;
        ...
        switch (filter.input) {
            ...
            case 'checkbox':
                Utils.iterateOptions(filter.values, function(key, val) {
                    h += '<div' + c + 'class="mini-checkbox" " name="' + name + '" id="'+name+'" trueValue="1"  falseValue="0" value="' + key + '" text="' + val + '"></div> ';
                });
                break;
            ...
            case 'textarea':
                h += '<textarea class="mini-textarea" name="' + name + '" id="'+name+'" ';
            ...
            case 'number':
                h += '<input class="mini-spinner"  name="' + name + '" id="'+name+'" ';
                if (validation.step !== undefined) h += ' increment="' + validation.step + '"';
                if (validation.min !== undefined) h += ' minValue="' + validation.min + '"';
                if (validation.max !== undefined) h += ' maxValue="' + validation.max + '"';
                //if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
            ...
            default:
                h += '<input class="mini-textbox"   name="' + name + '" id="'+name+'" ';
                //if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
...
QueryBuilder.define('invert', function(options) {
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            var $h = $(h.value);
            var _id= $h[0].id+"_invert_group";
            $h.find(Selectors.condition_container).after(
                '<a  class="mini-button" iconCls="fa-random" id="'+_id+'" name="_invert_group" data-invert="group" text="'+ self.translate('invert') +'">' +
                '</a>'
            );
            h.value = $h.prop('outerHTML');
        ...
        if (options.display_rules_button && options.invert_rules) {
            this.on('getRuleTemplate.filter', function(h) {
                ...
                $h.find(Selectors.rule_actions).prepend(
                    '<a class="mini-button" iconCls="icon-random" id="'+_id+'" name="_invert_rule" data-invert="rule" text="'+ self.translate('invert') +'">' +
                    '</a>'
                );
                h.value = $h.prop('outerHTML');
            });
        ...
        mini.parse();
       // var temp=mini.get('_invert_group');
        // temp.on('click',  function() {
        //     var $rule = $(this).closest(Selectors.rule_container);
        //     self.invert(self.getModel($rule), options);
        // });
        // temp=mini.get('_invert_rule');
        // temp.on('click',  function() {
        //     var $rule = $(this).closest(Selectors.rule_container);
        //     self.invert(self.getModel($rule), options);
        // });
        // var temp_controls = mini.getsbyName("_invert_group");
        //
        // $.each(temp_controls,function(i,item) {
        //     //var filterId = item.getId();
        //     item.on('click',  function() {
        ...
        //     });
        // });
        // temp_controls = mini.getsbyName("_invert_rule");
        //
        // $.each(temp_controls,function(i,item) {
        //     //var filterId = item.getId();
        //     item.on('click',  function() {
...
QueryBuilder.define('not-group', function(options) {
    ...
    if (!options.disable_template) {
        this.on('getGroupTemplate.filter', function(h) {
            var $h = $(h.value);
            var _id= $h[0].id+"_group_not";
            $h.find(QueryBuilder.selectors.condition_container).prepend(
                '<div   class="mini-checkbox"  id="'+_id+'" text="'+self.translate('NOT')+'">' +
                '</div>'
            );
            h.value = $h.prop('outerHTML');
        });
        ...
        var sql = (function parse(group) {
            ...
            group.rules.forEach(function(rule) {
                if (rule.rules && rule.rules.length > 0) {
                    ...
                    var filter={};
                    var matchingFilters = self.filters.filter(function(filter) {
                        return filter.field.toLowerCase() === rule.field.toLowerCase();
                    });


                    if (matchingFilters.length === 1) {
                        filter= matchingFilters[0] ;
                    }
                    if(rule.operator =='smart'){

                        var resWhere=parseQueryExpr(rule.value, filter.col);
                        if (resWhere!=undefined) {
                            if (ruleExpression!=undefined) {
                                ruleExpression = ruleExpression + ' AND (' + resWhere + ') ';
                            } else {
                                ruleExpression = '(' + resWhere + ') ';
                            }
                        }
                        ...
                        //treps
                        if( rule.tableName!= undefined  && rule.tableName!=''){
                            if(rule.idField!= undefined  && rule.idField!=''){
                                ruleExpression = ' EXISTS (SELECT 1 FROM '+rule.tableName+' WHERE '+rule.fK+' AND '+rule.idField+'=\''+rule.field+'\' AND '+rule.valueField + ' ' + sqlFn(value)+')';
                            }else{
                                ruleExpression = ' EXISTS (SELECT 1 FROM '+rule.tableName+' WHERE '+rule.fK+' AND '+rule.field + ' ' + sqlFn(value)+')';
                            }
