...
<script type="text/javascript">
	...
	${replace}Ckeditor.config.height =600;//<c:if test="${not empty uploadPath}">
    ...
    ${replace}Ckeditor.on('resize', function() {
       // alert(333);
        $("body").show();
    });

  //  CKEDITOR.instances["${replace}"].on("instanceReady", function() {
  //      var  ic=0;
        // 保存按钮
  //      this.addCommand("maximize", {
  //          modes : {
  //              wysiwyg : 1,
   //             source : 1
   //         },
   ...
   //             if(ic<=0){
   //                 ${replace}Ckeditor.fire("maximize", this.state);
   //                 $("body").show();
 //                   alert(111);
 //               }
 //               ic=ic+1;
