...
<c:forEach items="${fnc:getArticleList(category.site.id, category.id, not empty pageSize?pageSize:8, 'posid:2, orderBy: \"hits desc\"')}" var="article">
	<li class="zne_row clearfix">
	    <EM><fmt:formatDate value="${article.updateDate}" pattern="yyyy.MM.dd"/></EM>
		<a href="${ctx}/view-${article.category.id}-${article.id}${urlSuffix}" style="color:${article.color}" title="${article.title}">${fns:abbr(article.title,50)}
		</a>
	</li>
</c:forEach>