...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sys/org/list?id=${org.parent.id}&parentIds=${org.parentIds}">机构列表</a></li>
		<li class="active"><a href="${ctx}/sys/org/form?id=${org.id}&parent.id=${org.parent.id}">机构<shiro:hasPermission name="sys:org:edit">${not empty org.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:org:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="org" action="${ctx}/sys/org/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="org" name="parent.id" value="${org.parent.id}" labelName="parent.name" labelValue="${org.parent.name}"
					title="机构" url="/sys/org/treeData" extId="${org.id}" cssClass="" allowClear="${org.currentUser.admin}"/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="grade" class="input-medium">
					<form:options items="${fns:getDictList('sys_org_grade')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test="${empty org.id}">
			<div class="control-group">
				...
				<div class="controls">
					<form:checkboxes path="childDeptList" items="${fns:getDictList('sys_org_common')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</div>
			</div>
		</c:if>
