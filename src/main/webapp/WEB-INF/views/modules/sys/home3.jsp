            ...
            <!-- Main row -->
             <div class="row">
                <div class="col-md-7">
                    <div class="box box-info">
                        <div class="box-header with-border">
                            <h3 class="box-title"><a style = "cursor: pointer;" onclick="javascript:newTabPage('客户行业类别分析','/pt/ptCustomerCategoryAnalysis/view',true)">客户行业类别分析</a></h3>
                        ...
                        <!-- /.box-header -->
                        <div class="box-body">
                            <iframe id="khhylbfx" src="${ctx}/pt/ptCustomerCategoryAnalysis/view?noHeader=1" width="100%" height="320px"  scrolling="no" frameborder='0'></iframe>	
                        </div>
                    </div>
                </div>
                <!-- /.col -->
                   ...
                   for (var i = 0; i < data.length; i++) {
                	   ...
                	   if(title.length>16){
                	    ...
                	    if(row.categoryName=='国家政策'){ 
                      	 $('#cmsul').append("<li class=''><div class='product-img'><span class='label label-danger'>"+row.categoryName+"</span></div>"+
                     	  "<div class='product-info'><a href='javascript:void(0)' onclick=\"javascript:newTabPage('政策文件','${pageContext.request.contextPath}${fns:getFrontPath()}/view-"+row.categoryId+ "-" + row.id + "${fns:getUrlSuffix()}',true)\" class='product-title'>"+title+"<span class='pull-right'>"+row.updateDate.format('yyyy-MM-dd')+"["+row.publishName+"]</span></a></div></li>");
                	    }else{
                		   $('#cmsul').append("<li class=''><div class='product-img'><span class='label label-success'>"+row.categoryName+"</span></div>"+
                            "<div class='product-info'><a href='javascript:void(0)' onclick=\"javascript:newTabPage('政策文件','${pageContext.request.contextPath}${fns:getFrontPath()}/view-"+row.categoryId+ "-" + row.id + "${fns:getUrlSuffix()}',true)\" class='product-title'>"+title+"<span class='pull-right'>"+row.updateDate.format('yyyy-MM-dd')+"["+row.publishName+"]</span></a></div></li>"); 
                	   } 
                   }
               },
        ...
        $.ajax({
            ...
            success: function (text) {
                ...
                for (var i = 0; i < o.length; i++) {
                    ...
                    }else if(row.typeName=='省公司任务'||row.typeName=='本厂任务'){
                    	spancolor = 'label-warning';
                    }
                    $('#oaul').append("<li class=''><div class='product-img'><span class='label "+spancolor+"'>"+row.typeName+"</span></div><div class='product-info'><a href='javascript:void(0)' onclick=\"javascript:newTabPage('待办任务','${ctx}/oa/oaWorkPlan/form?action=view&id="+row.id+"&showForm=1&showList=0')\" class='product-title'>"+name+""+
                    	       "<span class='pull-right'>"+row.planStartDate.format('yyyy-MM-dd')+"</span></a></div></li>");
                }
