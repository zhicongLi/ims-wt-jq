		...
		$(document).ready(function() {
			...
			$("#inputForm").validate({
				submitHandler: function(form){
					loading('正在提交，请稍等...');
					//dmr:改为is(":checked"),原来attr(checked)无法保存
		...
		<c:otherwise>
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				...
				<fieldset>
					...
					<div class="control-group">
						<label class="control-label">父表关联主键:</label>
						<div class="controls">
							<form:input path="parentTableKeyField" htmlEscape="false" maxlength="50"  />
							<span class="help-inline">父表上,作为关联子表使用的字段</span>
						</div>
					...
					<div class="control-group">
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							<thead><tr><th title="数据库字段名">列名</th><th title="默认读取数据库字段备注">说明</th><th title="数据库中设置的字段类型及长度">物理类型</th><th title="实体对象的属性字段类型">Java类型</th>
								...
								<th title="选中后该字段被加入到明细表单里">表单显示</th>
								<th title="选中后该字段被加入到高级查询条件里">查询</th>
								<th title="选中后该字段被加入到表格查询框里">简要查询</th>
								<th title="选中后该字段被加入到快速定位条件里">快速定位</th>
								<th title="该字段为查询字段时的查询匹配放松">查询匹配方式</th>
								<th title="字段在表单中显示的类型">显示表单类型</th>
								<th title="显示表单类型设置为“下拉框、复选框、点选框”时，需设置字典的类型">字典类型</th>
								<th title="前端页面JS的验证规则vtype">验证规则</th>
								<th title="弹出选择窗口(LOV)URL地址">LOV地址</th>
								<th title="格式化字符串(日期格式)">格式</th>
								<th>排序</th></tr></thead>
							<tbody>
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isInForm" value="1" ${column.isInForm eq '1' ? 'checked' : ''}/>
									</td>
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isQuery" value="1" ${column.isQuery eq '1' ? 'checked' : ''}/>
									</td>
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isQuerySimple" value="1" ${column.isQuerySimple eq '1' ? 'checked' : ''}/>
									</td>
									<td>
										<input type="checkbox" name="columnList[${vs.index}].isKey" value="1" ${column.isKey eq '1' ? 'checked' : ''}/>
									</td>
									<td>
									...
									<td>
										<input type="text" name="columnList[${vs.index}].vtype" value="${column.vtype}" maxlength="200" class="input-mini"/>
									</td>
									<td>
										<input type="text" name="columnList[${vs.index}].lovUrl" value="${column.lovUrl}" maxlength="200" class="input-mini"/>
									</td>
									<td>
										<input type="text" name="columnList[${vs.index}].format" value="${column.format}" maxlength="200" class="input-mini"/>
									</td>
									<td>
